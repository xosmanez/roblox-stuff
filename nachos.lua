...
-- createhitelement from cripbot-paste :3 sucking nixus diccck
...
local trolling = exploits:Sector("trolling", "Left")
        trolling:Element("Toggle", "fake angles",{}, function(val) -- fr
		...
		trolling:Element("ToggleKeybind", "ddos")
		--[[trolling:Element("Toggle", "auto respawn",{}, function(val) -- fkin buggy....
			if val.Toggle then 
				while wait(0.172) do
		...
		trolling:Element("Button", "respawn",{}, function()
			game.ReplicatedStorage.Events:FindFirstChild("Spawnme"):FireServer()
        end)
        trolling:Element("Button", "godmode",{}, function()
            game.Players.LocalPlayer.Character.Humanoid:Remove()
            Instance.new('Humanoid', game.Players.LocalPlayer.Character)
        end)
		trolling:Element("Button", "break cheats", {}, function()
			LocalPlayer.Character.UpperTorso.Waist:Destroy()
		end)
		trolling:Element("Button", "rejoin", {}, function()
			game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayer)
		end)
		trolling:Element("Toggle", "md") 
...
kbot:Element("Dropdown", "knifebot type", {options = {"standart", "aura", "Hittin P"}})
kbot:Element("Slider", "Knifebot Radius", {min = 0, max = 1000, default = 20})
kbot:Element("Toggle", "knifebot ignore map", {default = {Toggle = false}})

local something = exploits:Sector("something", "Left")
something:Element("ToggleKeybind", "hps kill all")   
something:Element("ToggleKeybind", "kill all")     
something:Element("ToggleKeybind", "best kill all")
...
something:Element("Slider", "hits amount", {min = 1, max = 60, default = 1})  
...
local aimbot = rage:Sector("aimbot", "Left")      
		...
		aimbot:Element('Slider', 'hitscan', {min = 1, max = 20, default = 3}) -- 3 is a perfect hitscan!  
		aimbot:Element("Toggle", "silent aim")    
		aimbot:Element("Toggle", "simulate shoot") -- tip: if u not gonna toggle this then u gonna have gun-aura :money: 
		aimbot:Element("Toggle", "force hit")    
		aimbot:Element("Toggle", "prediction") 
		aimbot:Element("Toggle", "nan prediction") 
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "zero", "among us", "smooth"}})
...
local exploits = rage:Sector("exploits", "Left")      
		...
		exploits:Element("ToggleKeybind", "Forward Trinking") 
	...
	for _,Player in pairs(Players:GetPlayers()) do
		if Player.Character then
			for _2,Obj in pairs(Player.Character:GetDescendants()) do
				if Obj.Name == "VisibleCham" or Obj.Name == "WallsCham" then
					if tbl.Toggle then
					...
					local nigger1 = true
					   if nigger1 == true then
						...
						mt.__namecall = newcclosure(function(self, ...)
							local args = {...}
							if getnamecallmethod() == "FireServer" and self.Name == "Hit" and tick() - lasthittick > 0.005 then
								lasthittick = tick()
								spawn(function()
...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "minecraft xp"}}) 
world:Element("Dropdown", "killsound", {options = {"none", "neverlose", "csgo", "ultrakill", "killingspree"}}) 			
...
movement:Element("Dropdown", "type", {options = {"gyro", "cframe", "crim hop"}})
			...
			chat:Element("Toggle", "chat spam", nil, function(tbl)
				if tbl.Toggle then
					while values.misc.chat["chat spam"].Toggle do
						game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Standard" and spammessages[math.random(1,table.getn(spammessages))] or values.misc.chat.type.Dropdown == "cuteware" and cwspammsg[math.random(1,table.getn(cwspammsg))] or values.misc.chat.type.Dropdown == "trash facts(RU)" and trashfacts[math.random(1,table.getn(trashfacts))], false, "Innocent", false, true)
						wait(values.misc.chat["speed (ms)"].Slider/1000)
					end
				end
			end)
			chat:Element("Dropdown", "type", {options = {"Standard", "cuteware", "trash facts(RU)"}})
chat:Element("Slider", "speed (ms)", {min = 10, max = 1000, default = 500})
chat:Element("Toggle", "kill say")
chat:Element("Toggle", "hit say")
chat:Element("Toggle", "randomized kill say")
chat:Element("Dropdown", "kill say type", {options = {"cripbot", "rosense", "nachos"}})
...
cleaner:Element("Toggle","clear children",{},function(tbl)
	if tbl.Toggle then
...
for i, v in next, game.ReplicatedStorage.Sounds:GetChildren() do  
	if v.Name == "Headshot4" or v.Name == "Headshot3" or v.Name == "Headshot2" or v.Name == "Headshot1" or v.Name == "Helmet4" or v.Name == "Helmet3" or v.Name == "Helmet2" or v.Name == "Helmet1" or v.Name == "HHeadshot4" or v.Name == "HHeadshot3" or v.Name == "HHeadshot2" or v.Name == "HHeadshot1" or v.Name == "Concrete1" or v.Name == "Kevlar1" or v.Name == "Kevlar5" or v.Name == "Kevlar2" or v.Name == "Kevlar3" or v.Name == "Kevlar4" or v.Name == "Bullet1" or v.Name == "Bullet2" or v.Name == "Bullet3" or v.Name == "Bullet4" or v.Name == "Concrete4" or v.Name == "Concrete2" or v.Name == "Concrete3"  then
		v:Destroy()  
	end
end
end
...
local lastShotTime = 2 -- nixus i luv u :heart: -- its buggy btw
        local hpsLimit = 3
        local hitsPerSecond = 3
        local connection
        connection = RunService.Heartbeat:Connect(function()
        if values.exploits.something["hps kill all"].Toggle and values.exploits.something["hps kill all"].Active and LocalPlayer.Character:FindFirstChild("Gun") then 
        local currentTime = tick()
        if currentTime - lastShotTime >= (1/hpsLimit) then
            lastShotTime = currentTime
            for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Character:FindFirstChild("UpperTorso") and Player.Parent.LocalPlayer.Character and Player.Parent.LocalPlayer.Character:FindFirstChild("EquippedTool") then
...
RunService.Heartbeat:Connect(function() -- from old shootware -- i love u stix!!
...
RunService.RenderStepped:Connect(function(step)
	...
	if PlayerIsAlive then      
		...
		if values.exploits.trolling["ddos"].Toggle and values.exploits.trolling["ddos"].Active then      
			for count = 1, 77, 1  do      
		...
		if values.exploits.something["best kill all"].Toggle and values.exploits.something["best kill all"].Active and LocalPlayer.Character:FindFirstChild("UpperTorso") and LocalPlayer.Character:FindFirstChild("Gun") then
			for b2, b3 in pairs(game:GetService("Players"):GetChildren()) do
				if b3.Team ~= b3.Parent.LocalPlayer.Team then
					if b3.Character and b3.Character:FindFirstChild("UpperTorso") and b3.Parent.LocalPlayer.Character and b3.Parent.LocalPlayer.Character:FindFirstChild("EquippedTool") then
						if b3.Character:FindFirstChild("Humanoid") and b3.Character.Humanoid.Health > 0 then
		...
		if workspace:FindFirstChild("Map") and Client.gun ~= "none" and Client.gun.Name ~= "C4" then      
			...
			if values.rage.aimbot.enabled.Toggle then      
				...
				for _,Player in pairs(Players:GetPlayers()) do  
					if TBLFIND(values.misc.client["gun modifiers"].Jumbobox, "kniferate") and Client.gun:FindFirstChild("Melee") then -- huh
						Client.DISABLED = false 
					end    
					if TBLFIND(values.misc.client["gun modifiers"].Jumbobox, "firerate") then      
						Client.DISABLED = false      
					...
					spawn(function()
						...
						if values.rage.exploits['Forward Trinking'].Toggle then
							if Player.Team ~= LocalPlayer.Team then
							  if (Player.Character.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude < 70  then 
								Player.Character.Head.Position = game.Players.LocalPlayer.Character.HumanoidRootPart.Position+Vector3.new(math.random(1,10)) 
							   end
							end
						end
					end)    
					...
					if Client.gun:FindFirstChild("Melee") and values.exploits.kbot["knifebot"].Toggle then      
						local Ignore = {unpack(Collision)}      
								...
								local Ignore = {unpack(Collision)}      
								if values.exploits.kbot["knifebot ignore map"].Toggle then
		...
		BodyVelocity.MaxForce = Vec3(HUGE,0,HUGE) -- no brain
		...
		if not UserInputService:IsKeyDown("Space") and values.misc.movement["walkspeed"].Toggle and values.misc.movement["walkspeed"].Active then      
			...
			BodyVelocity.Parent = LocalPlayer.Character.UpperTorso      
			--LocalPlayer.Character.Humanoid.Jump = true      
			BodyVelocity.Velocity = Vec3(rot.LookVector.X,0,rot.LookVector.Z) * (values.misc.movement["speedo"].Slider * 2)      
			if add == 0 and direct == "directional" and not UserInputService:IsKeyDown("W") then      
				...
				if values.misc.movement["walk type"].Dropdown == "cframe" then      
					BodyVelocity:Destroy()      
					Root.CFrame = Root.CFrame + Vec3(rot.LookVector.X,0,rot.LookVector.Z) * values.misc.movement["speedo"].Slider/50      
				end      
			end      
		end      
...
mt.__namecall = function(self, ...)
	...
	if method == "FireServer" then
		...
		if self.Name == "ReplicateCamera" and values.misc.client["anti spectate"].Toggle then
			args[1] = CFrame.new(Vector3.new(math.random(999e999, 9999e9999), math.random(999e999, 9999e9999), math.random(999e999, 9999e9999)))
		end
	...
	if method == "FireServer" and self.Name == "Hit" then
		...
		if values.exploits.trolling["md"].Toggle then 
			args[8] = 9e9 * 9e9 * 9e9
		end 
		...
		if values.rage.aimbot["nan prediction"].Toggle and RageTarget ~= nil then
			coroutine.wrap(function()
				if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then 
...
local rosense = {"femboy'd";"delete ROBLOX bro";"shippy#0001";"ur kd is 0-20 ?";"r u femboy?";"bloxsense.pub";"cry cry cry cry cry";"rosensed";"1 by rosense";"invaded tap";"bloxsense.lol";"sorry pls"}
		...
		LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
			if current == 0 then return end      
				if values.misc.chat["randomized kill say"].Toggle and current ~= 0 then 
					...
					elseif values.misc.chat["kill say type"].Dropdown == "rosense" then
						game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(rosense[math.random(1, #rosense)], false, "Innocent", false, true)
...
astcurrent = LocalPlayer.Status.Kills.Value
LocalPlayer.Status.Kills:GetPropertyChangedSignal('Value'):Connect(function(current)
	if current == 0 then return end
		...
		if Character ~= nil then
			...
			for _,obj in pairs(Character:GetChildren()) do
				if obj:IsA("BasePart") and Player ~= LocalPlayer and obj.Name ~= "HumanoidRootPart" and obj.Name ~= "Head" and obj.Name ~= "BackC4" and obj.Name ~= "HeadHB" then
					...
					local WallsCham = Instance.new("BoxHandleAdornment")
					WallsCham.Name = "WallsCham"
					WallsCham.AlwaysOnTop = true
					WallsCham.ZIndex = 5
					WallsCham.Size = obj.Size + Vector3.new(0.1,0.1,0.1)
					WallsCham.AlwaysOnTop = true
					WallsCham.Transparency = 0.7

					if values.visuals.players.chams.Toggle then
						if values.visuals.players.teammates.Toggle or Player.Team ~= LocalPlayer.Team then
							VisibleCham.Visible = true
							WallsCham.Visible = true
						else
							VisibleCham.Visible = false
							WallsCham.Visible = false
						end
					else
						VisibleCham.Visible = false
						WallsCham.Visible = false
					...
					table.insert(ChamItems, WallsCham)

					VisibleCham.Color3 = values.visuals.players.chams.Color
					WallsCham.Color3 = values.visuals.players.chams.Color

					VisibleCham.AdornCullingMode = "Never"
					WallsCham.AdornCullingMode = "Never"

					VisibleCham.Adornee = obj
					VisibleCham.Parent = obj

					WallsCham.Adornee = obj
					WallsCham.Parent = obj
				end
			end
		end
    end)
...
CreateHitElement('loaded in ' .. math.round((tick() - loaded)*10)/10 .. ' second(s)',Color3.new(1,1,1), 5, 0, 270,  0, 22)
CreateHitElement("press delete to open and close menu",Color3.new(1,1,1), 3, 0, 280, 0, 22)

-- creds to: stormy, suka.blyat, lio, nixus, stix, boui and dev xosmaneeeee
