-- hello can u paste my features pls?
--rconsolename'i love memz'
local teckosaweg = tick()
local esp = loadstring(game:HttpGet(('https://pastebin.com/raw/LDM950At'), true))();
--local esp = loadstring(game:HttpGet(('https://raw.githubusercontent.com/Maks17chetkijZ/espz/main/espp.lua'), true))();
...
local nixus = {
  ...
  "это 1 еблан😘",
  "Varechkaa#2007",
  "мой килл сей лучше твоего(",
  "you have 20501 fathers",
  ...
  "u got neverbloxed",
  "1 by DeathCore",
  "you have 2012 fathers, deathcore said so",
   "p100",
   "hitting big p",
   "ez kills",
   "ez wins",
...
local DeadInside = {"what are you feel?";"baby, what you wishin’ for?";"это 1 еблан😘";"Varechkaa#2007";"мой килл сей лучше твоего(";"you have 20501 fathers";"Maybe you should wish it more?";"The unfolding of the year";"M.I.A";"touch my pussy";"will we still have learned it?";"ты сдох чел(";"give me bloxsense.pub";"hi semi";"1 by hui.ware";"1 by varya.ware";"backtacke'd";"1000-7";"zxc?";"wanna sex";"i feel bad";"u got neverbloxed";"1 by DeathCore";"you have 2012 fathers, deathcore said so"} 
local facts = {
           -- facts (ru)
...
runService.RenderStepped:Connect(function(step)
    if Library.Watermark.Visible == true then
        ...
        if IsAlive(localPlayer) then
            ...
            Library:SetWatermark('RussianWare | fps: ' .. fps .. ' | health: ' .. health .. " | kills: " .. killls .. " | speed: " .. speed .. ' | kd/r:' .. kdr)
        else
            Library:SetWatermark('RussianWare | fps: ' .. fps .. ' | kills: ' .. killls .. ' | kd/r:' .. kdr) -- if died
        end
    end
end)
...
LeftMenuGroup:AddButton('Copy Discord Invite', function()
    setclipboard("discord.gg/4NmWfzvMPx")
end)
...
Sections.fwt:AddSlider('rage_fwtAmount', {Text = 'Amount' ,Default = 0, Min = 0, Max = 10, Rounding = 2, Compact = false})

-- //

Sections.backtr:AddToggle('rage_backtracks', {Text = 'Backtrack'})
Sections.backtr:AddSlider('rage_backtrack', {Text = 'In Varya' ,Default = 75, Min = 0, Max = 350, Rounding = 0, Compact = false})
...
Sections.Tweaks:AddToggle('RemoveDmgFromFire', {Text = 'Remove Burn Damage', Tooltip = 'died'})
Sections.Tweaks:AddToggle('RemoveDmgFromFall', {Text = 'Remove Fall Damage', Tooltip = 'died too'})
...
Sections.Client:AddToggle('hbbriker', {Text = 'Hitbox Breaker', Default = false, Risky = true})
...
Sections.Chat:AddDropdown('KillType', {Text = 'Say Type', Values = {'Lio', 'Nixus', 'Dead-Inside', 'Xosmane', 'CS:GO HvH', 'China Propaganda', 'Random Words', 'Facts'}, Default = 1, Multi = false})
...
Sections.Desyncsec:AddSlider('desync_random_x', {Text = 'Random X', Min = 0, Max = 9999e999, Rounding = 0, Default = 9999e999})
Sections.Desyncsec:AddSlider('desync_random_y', {Text = 'Random Y', Min = 0, Max = 35, Rounding = 0, Default = 1})
Sections.Desyncsec:AddSlider('desync_random_z', {Text = 'Random Z', Min = 0, Max = 999e99, Rounding = 0, Default = 999e99})
...
pcall(function()
    if IsAlive(lplr) and workspace.Map.Gamemode.Value == "defusal" and workspace.Status.Preparation.Value == false and not planting then 
        ...
        wait(0.2)
        PlantC4:FireServer((pos + Vector3.new(1000, 1000, 1000)) * CFrame.Angles(math.rad(-math.huge), 0, math.rad(math.huge)), GetSite())
        wait(0.2)
...
Sections.Local:AddSlider('fov_amount', {Text = 'Field of view', Min = 60, Max = 120, Rounding = 0, Default = 75})
...
localPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(val)
    ...
    elseif Options and Options.KillType and Options.KillType.Value == 'Dead-Inside' then
        game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(DeadInside[math.random(1, #DeadInside)], false, false, true)
...
local adss = {
    "gg/4NmWfzvMPx",
    "hey u should join gg/4NmWfzvMPx",
    'russianware',
...
function encodePos(L_946_arg0) -- still using encodePos?
...
if IsAlive(localPlayer) then
    if Toggles.KillAll.Value and localPlayer.Character:FindFirstChild("Gun") then -- nixus was here <3
            ...
            for _, plr in pairs(game.Players:GetChildren()) do
                if plr.Character and plr.Team ~= localPlayer.Team and IsAlive(plr) then
                    ...
                    for i = 0,2 do
                        ...
                        local oh2 = Vector3.new(0/0, 0/0, 0/0)
                        local oh3 = 'Bayonet'
                        local oh4 = 0
                        local oh5 = nil
                        local oh6 = nil
                        local oh8 = 999e99
                        local oh9 = Toggles.killAll_arg9.Value
        ...
        if Toggles.hbbriker.Value and LocalPlayer.Character:FindFirstChild("UpperTorso") and LocalPlayer.Character:FindFirstChild("Gun") then  -- stix x infinity ?
            for _,Player in pairs(Players:GetPlayers()) do
                if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild('UpperTorso') then
                    local oh1 = Player.Character.LowerTorso
                    ...
                    local oh16 = nil
                    local oh17 = nil
                    game:GetService("ReplicatedStorage").Events.Hit:FireServer(oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13, oh14, oh15, oh16, oh17)
                end
            end
        end
...
if Toggles.Third_Person.Value and Options.Third_Bind:GetState() then
    ...
    for _, plr in pairs(plrs:GetPlayers()) do
        ...
        camera.FieldOfView = Options.fov_amount.Value
...
runService.Heartbeat:Connect(function()
    if Toggles.hbbriker.Value and LocalPlayer.Character:FindFirstChild('Gun') then
        for _,Player in pairs(Players:GetPlayers()) do
                                if Player.Character and Player.Team ~= LocalPlayer.Team then
                                    ...
                                    local oh2 = Encrypt(oh1.Position)
                                    local oh3 = 'Knives'
                                    local oh4 = -8888
                                    local oh5 = LocalPlayer.Character.Gun
                                    local oh7 = 77765
    ...
    local hackspeed = Options.SpeedValue.Value
    ...
    if Toggles.InfAmmo.Value then 
      ...
      --  end
        ...
        if enabled and IsAlive(localPlayer) then
             if Options.SpeedType.Value == 'Velocity' then
        ...
        elseif IsAlive(localPlayer) then 
			...
			if IsAlive(lplr) then
				if Toggles.desync_enabled.Value and not exclude["var1"] then
						...
						hrpcframeclone = hrpcframeclone * CFrame.new(RandomVectorRange(Options.desync_random_x.Value, Options.desync_random_y.Value, Options.desync_random_z.Value))
						end
		...
		utility:BindToRenderStep("desync_1", 1, function()       if exclude[1] and exclude["var1"] and IsAlive(lplr) then                lplr.Character.HumanoidRootPart.CFrame = exclude[1]         exclude["var1"] = false           end    end )
...
local Hit = game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("Hit")
local namecall
namecall = hookmetamethod(game, "__namecall", function(self, ...)
    local args = {...}
    local method = getnamecallmethod():lower()
    local oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13, oh14, oh15, oh16, oh17 = ...
    if self == Hit and method == "fireserver" and Toggles.HuiPart.Value and ragebot_target ~= nil then
		oh2 = not Toggles.NaN_Predict.Value and oh2 or oh2 + Vector3.new(0/0,0/0)
        oh3 = not Toggles.Custom_Gun.Value and Client.gun.Name or Options.killgun.Value
        oh4 = -4000
        oh5 = nil
        oh6 = nil
        oh8 = not Toggles.infinity_damage.Value and 1 or 9e9 * 9e9 * 9e9 -- getgenv().varyshkas
        oh9 = not Toggles.rbm_oh.Value and false or true
        oh10 = not Toggles.wallbang_oh.Value and false or true
        oh11 = Vector3.new(0,0,0)
        oh12 = -85428592
        oh13 = Vector3.new(0/0,0/0)
        return namecall(self, oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13)
    end
...
def = hookmetamethod(game, "__namecall", function(self,...)
    ...
    if meth == "SetPrimaryPartCFrame" and self.Name == "Arms" then
       ...
       if Toggles.VizSilentAngle.Value and ragebot_target then
        args[1] = CFrame.lookAt(args[1].p, ragebot_target.Position) 
      end
...
ThemeManager:SetFolder('russianware')
SaveManager:SetFolder('russianware/Counter Blox')
